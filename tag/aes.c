...
int Aes128EncryptECB( const unsigned char key128[16], const unsigned char plaintext[16], unsigned char ciphertext[16] )
{
    ...
    int ret = AesSetkeyEnc(&aesContext, key128, 128);
    ret |= AesCryptECB(&aesContext, AES_ENCRYPT, plaintext, ciphertext);
    return ret;
}
...
{
    AesContext aesContext;
    int ret = AesSetkeyDec(&aesContext, key128, 128);
    ret |= AesCryptECB(&aesContext, AES_DECRYPT, ciphertext, plaintext);
    return ret;
}
...
int Aes128EncryptCBC( const unsigned char key128[16], const unsigned char iv[16], unsigned int length, const unsigned char* plaintext, unsigned char* ciphertext )
{
    AesContext aesContext;
    int ret = AesSetkeyEnc(&aesContext, key128, 128);
    ret |= AesCryptCBC(&aesContext, AES_ENCRYPT, length, iv, plaintext, ciphertext);
    return ret;
}
...
int Aes128DecryptCBC( const unsigned char key128[16], const unsigned char iv[16], unsigned int length,  const unsigned char* ciphertext, unsigned char* plaintext )
{
    AesContext aesContext;
    int ret = AesSetkeyDec(&aesContext, key128, 128);
    ret |= AesCryptCBC(&aesContext, AES_DECRYPT, length, iv, ciphertext, plaintext);
    return ret;
}
 ...
 */
int AesSetkeyEnc( AesContext *ctx, const unsigned char *key, unsigned int keybits )
{
    unsigned int i;
    unsigned long *RK;
    ...
    switch( keybits )
    {
        ...
        default : return( ERROR_AES_INVALID_KEY_LENGTH );
    }
...
int AesSetkeyDec( AesContext *ctx, const unsigned char *key, unsigned int keysize )
{
    ...
    ret = AesSetkeyEnc( &cty, key, keysize );
    if( ret != 0 )
        return( ret );
    ...
    *RK++ = *SK++;
    ...
    memset( &cty, 0, sizeof( AesContext ) );
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return( ERROR_AES_INVALID_INPUT_LENGTH );
    memcpy(tmpIV, iv, 16);
    ...
    if( mode == AES_DECRYPT )
    {
        while( length > 0 )
        {
            ...
            AesCryptECB( ctx, mode, input, output );

            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( output[i] ^ tmpIV[i] );

            memcpy( tmpIV, temp, 16 );
    ...
    else
    {
        ...
        {
            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( input[i] ^ tmpIV[i] );

            AesCryptECB( ctx, mode, output, output );
            memcpy( tmpIV, output, 16 );
